...
class UserApiController extends Controller
{
    ...
    public function apiDoctors(Request $request)
    {
        ...
        $doctor = Doctor::with('treatment:id,name')->whereStatus(1)->where('is_filled',1)->whereSubscriptionStatus(1)->get();
        $data = $request->all();
        $doctors = $this->getNearDoctor($doctor,$data['lat'],$data['lang']);
        foreach ($doctors as $doctor)
        {
            $doctor->is_faviroute = $this->checkFavourite($doctor->id);
             $doctor['reviews'] = Review::with('user:id,name,image')->where('doctor_id',$doctor->id)->get();
               $doctor['tiz'] = Hospital::where('id',$doctor->hospital_id)->first();
           // unset($doctor->hospital_id);
        }
    ...
    public function apiSingleDoctor(Request $request,$id)
    {
     ...
     /*   $request->validate([
       ...
       public function amgadapiSingleDoctor(Request $request,$id)
    {
      ...
      public function amgad()
    {
        ...
        $past = array();
        $appoint = array();
        $appointments = Appointment::with(['hospital:id,address,name'])->where('user_id',30)->orderBy('id','DESC')->get(['id','date','time','appointment_status','patient_name','doctor_id','appointment_id','hospital_id']);
        foreach($appointments as $appointment)
        {
            if ($appointment->hospital == null) {
                unset($appointment['hospital']);
            ...
            if (!empty(array_intersect($hospitals, $h)))
            {
                ...
                foreach($hss as $hospital)
                {
                    ...
                    $temp['hospital_name'] = $h->name;
                     $temp['hospital_address'] = $h->address;
                    array_push($array,$temp);
                    $d->hospital_id = $array;
                }
