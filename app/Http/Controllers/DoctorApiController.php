...
class DoctorApiController extends Controller
{
    ...
    {
        $doctors = Doctor::where('treatment_id ', $request->treatment_id  )->where('category_id ', $request->category_id  )->get();
    ...
    public function apiDoctorRegister(Request $request)
    {
       ...
       $test= $request->validate([
        ...
        for($i = 0; $i < count($days); $i++)
        {
          ...
          $id = Doctor::where('user_id',$user->id)->first();
        $request->validate([
           // 'name' => 'bail|required',
        ...
        //amgad register edit for tharwat
        ...
        if($user->verify == 1)
        {
            if (Auth::attempt(['email' => $user['email'], 'password' => $request->password]))
            {
                ...
                $userDoctor['is_filled'] = $doctor->is_filled;
                   $userDoctor['user_id'] = $user->id;//amgad
       ...
       public function apiUpdateDoctoramgad(Request $request)
    {    
        
        $id = Doctor::where('user_id',$request->user_id)->first(); 
         // return response(['success' => $request->user_id, 'msg' => 'successfully Update']);
        $request->validate([
            'name' => 'bail|required',
            'treatment_id' => 'bail|required',
            'category_id' => 'bail|required',
    ...
    public function apiDoctorAppointment()
    {
       ...
       public function amgadapiDoctorAppointment()
    {

        $doctor = Doctor::where('user_id',23)->first();
    ...
    public function apiLoginDoctor(Request $request)
    {
      ...
      public function amgadapiLoginDoctor(Request $request)
    {
        $doctor = Doctor::where('user_id',23)->first()->makeHidden(['rate','review']);
        $doctor->email = User::find($doctor->user_id)->email;
        $doctor->phone = User::find($doctor->user_id)->phone;
        $token = User::find($doctor->user_id)->agora_token;
        $cn = User::find($doctor->user_id)->channel_name;
    ...
    //
     ...
     public function amgadapiNotification()
    {
        $doctor = Doctor::where('user_id',auth()->user()->id)->first();
        $data = Notification::with('user:id,name,image')->get();
        return response(['success' => true , 'data' => $data]);
    }
    ...
    //
     ...
     public function amgadapiCancelAppointment()
    {
        $doctor = Doctor::with('treatment:id,name')->where('user_id',auth()->user()->id)->first();
        $cancel_appointment = Appointment::with('user:id,image')->get(['id','date','time','user_id','patient_address','patient_name','age','amount']);
        return response(['success' => true , 'data' => $cancel_appointment]);
    }
